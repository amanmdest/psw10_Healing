...
from medico.models import DadosMedico, DataAberta, Especialidade, is_medico
...
def escolher_horario(request, id_dados_medicos):
    if request.method == "GET":
        ...
        datas_abertas = DataAberta.objects.filter(user=medico.user).filter(data__gte=datetime.now()).filter(agendado=False)
...
def agendar_horario(request, id_data_aberta):
    if request.method == "GET":
        data_aberta = DataAberta.objects.get(id=id_data_aberta)
